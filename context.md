...
- Project moved into git and pushed to GitHub (`https://github.com/hammadsaleemch345/kumo-kosmos-backend`), so it can be worked on from a second laptop — this had never been version controlled before, everything was edited directly on disk. Repo is **public**: the raw Substack export (`posts.csv`, `email_list.kumokosmos.csv`, `posts/`, ~2785 real subscriber emails plus the client's paid content) was deliberately excluded via `.gitignore` rather than pushed, flagged this risk before pushing rather than assuming it was fine
  - Note for future rebases: the first push hit a conflict against GitHub's auto-generated placeholder README, and `git checkout --ours README.md` during a *rebase* keeps the upstream side (the placeholder), not the local commit — the reverse of a normal merge. That silently wiped the real README; caught and fixed in a follow-up commit
- CCBill's reviewer came back saying they couldn't see what content the anthology purchase actually unlocks. Root cause was presentational, not missing functionality: on the product page the included stories rendered as muted grey text with no hover state or click affordance, so the list read as a caption rather than the purchasable contents, and nothing on the page connected the list to the purchase. Fixed in `renderProduct()` + a new `.included-story` CSS class: each story is now a bordered card with a bright bold title, an explicit "Read this story →" cue, and a gold hover border; heading changed from "Stories in this collection" to "What you get with this purchase" with a line above the list stating that buying unlocks full access to them. Deployed and verified live (served file byte-identical to local)
- Client asked whether the browser back button could work (it did nothing, or dumped them back to the search engine they arrived from). Root cause was mine: the URL routing added in August used `history.replaceState()`, which overwrites the current history entry rather than adding one, so the site only ever occupied a single entry. Deliberate at the time to avoid back-button spam from internal `go()` calls, but checking the call sites showed all of them are genuine user navigations, so the concern didn't apply. Switched to `pushState` (renamed `updateUrlHash` to `pushUrlHash`) plus a `popstate` listener, with `go()` taking an optional `fromHistory` flag so back/forward re-render without pushing another entry. Unchanged-hash guard means clicking through to the page you're already on doesn't cost a second press of back. Verified with a stubbed history harness before deploying, not just eyeballed
  - Still not restored by back: the browse page's search query and tag filters, which live in the DOM/`activeTags` rather than the URL. Putting them in the URL is a bigger change (needs a decision on whether typing counts as a history entry or only an explicit search), flagged to the client as a follow-up rather than assumed
